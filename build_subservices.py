...
# Reuse the service_page template from build_services.py
ns = {}
exec(open('build_services.py').read().split("SERVICES = [")[0], globals())
...
def S(slug, title, strap, img, i1, i2, inc, s1, s2, s3, why, q1, a1, q2, a2, proj=None):
    ...
    return dict(slug=slug, title=title, strap=strap, img=IMG[img],
        ...
        faqs=[(q1, a1), (q2, a2)],
        guide=(
            '../blog/planning-permission-rear-extension.html' if 'extension' in slug or slug in ('sunroom','conservatory','outbuild-design') else
            '../blog/conservation-area-permitted-development.html' if slug in ('loft-conversion','garage-conversion','hmo-conversion','barn-conversion') else
            '../blog/feasibility-studies-before-you-buy.html' if slug in ('new-homes','basement-conversion','smart-flat-conversion','property-condition-survey') else
            '../blog/principal-designer-explained.html' if slug in ('structural-calculations','wall-removal','chimney-removal') else
            '../blog.html'))
...
SUBS = [
S('rear-extension','Rear Extensions','Single and double-storey rear extensions across London, from permitted development checks to completion on site.','p5',
 "A rear extension is the most common way London homes gain space, and the rules around it are precise: permitted development allows 3 metres of depth on an attached house and 4 on a detached one, with larger schemes possible through prior approval.",
